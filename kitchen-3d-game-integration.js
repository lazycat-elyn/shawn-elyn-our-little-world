(()=>{
  ...
  function ensureFrame(){
    ...
    frame.title='Kitchen 3D Visual Rebuild 2.1';
    frame.src='./kitchen-3d-modular-2.html?embed=1&v=2100';
    frame.allow='fullscreen';
    frame.addEventListener('load',()=>setTimeout(()=>sendState(frame.contentWindow),120));
    badge=document.createElement('div');
    badge.className='k3d-game-badge';
    badge.textContent='KITCHEN 3D VISUAL REBUILD 2.1 · HIGH FIDELITY · 🛍️ Shop · 📦 Inventory';
  ...
  window.addEventListener('message',e=>{
    ...
    if(d.type==='kitchen-appliance-buy'){
      ...
      s.coins-=cost;s.kitchenAppliancesOwned.push(id);s.kitchenApplianceLayout[id]=s.kitchenApplianceLayout[id]||{placed:false};
      if(typeof save==='function')save();if(typeof renderTaskUI==='function')renderTaskUI();if(typeof rewardPop==='function')rewardPop(`🛍️ 买到 ${LABEL[id]} · -${cost} Coins`);
      sendToKitchen({type:'kitchen-appliance-buy-result',ok:true,id},e.source);sendState(e.source);return;
    }
  ...
  window.KITCHEN_3D_GAME_INTEGRATION={version:'2.1',show:showKitchen3D,hide:hideKitchen3D,syncState:sendState};
})();
