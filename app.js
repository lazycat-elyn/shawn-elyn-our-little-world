...
const CAR2_PROFILES={
 easy:{label:'轻松',sub:'转向较稳 · 事件较少',traffic:.72,eventGap:1.15,penalty:.62,steer:.78,grip:1.06,parkTol:1.18,maxSpeed:118},
 normal:{label:'标准',sub:'推荐 · 真实但不刁难',traffic:1,eventGap:.88,penalty:1,steer:1,grip:1,parkTol:1,maxSpeed:138},
 hard:{label:'挑战',sub:'车流更多 · 规则更严格',traffic:1.28,eventGap:.68,penalty:1.28,steer:1.10,grip:.93,parkTol:.88,maxSpeed:150}
};
...
function startCarDrive10(carId,dest,driver,together,tripInit){
 ...
 const goodEvent=msg=>{metrics.eventsGood++;flashDrive('✓ '+msg);driveTone(760,.06,.03,'sine')};
 const badEvent=(pen,msg)=>{pen*=profile.penalty;metrics.eventsBad++;metrics.rules=Math.max(0,metrics.rules-pen);metrics.safety=Math.max(0,metrics.safety-pen*.68);flashDrive(msg);driveTone(110,.15,.08)};
